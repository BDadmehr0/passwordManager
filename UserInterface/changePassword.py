...
class ChangePassword(QWidget):
    submitClicked = pyqtSignal(int, str)

    def __init__(self, appVersion: str, screenWidth: int, screenHeight: int, dataFilesPath: str, mainPasswordFileName: str, parent=None) -> None:
    ...
    def changePassword(self) -> None:
        file = open(Path(self.dataFilesPath, self.mainPasswordFileName), "r", encoding="utf-8")
        data = json.load(file)
        file.close()
        for row in data:
            if row == "mainPassword":
                if self.oldPasswordLineEdit.text() == data[row]:
                    if self.newPasswordLineEdit.text() == self.confirmPasswordLineEdit.text():
                        self.submitClicked.emit(0, self.newPasswordLineEdit.text())
                        self.closeWindow()
                    else:
                        self.alertWindow("new and confirm passwords are not the same!")
                        self.submitClicked.emit(1, "")
                        self.closeWindow()
                else:
                    self.alertWindow("old password is not correct!")
                    self.submitClicked.emit(2, "")
                    self.closeWindow()
